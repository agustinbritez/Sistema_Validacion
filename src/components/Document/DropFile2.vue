<template>
  ...
  >
        ...
        <ul>
          <li v-for="(value, index) in uploadedFiles" :key="index">
            ...
            <button
              ...
              v-on:click.stop.prevent="removeFile"
            >
              <span
    ...
    <div
      class="add-btn"
      v-if="uploadedFiles.length > 0 && uploadedFiles.length <= limit - 1"
    >
      <a
        class="btn btn-default btn-pill"
        href="#"
        v-on:click.stop.prevent="addFile"
      >
        <span
    ...
    <div v-if="uploadedFiles.length > 0" class="cta">
      <button
        class="btn btn-lg btn-success btn-pill btn-cta"
        v-on:click="verify()"
        v-html="this.lb_13"
      ></button>
      <button
        class="btn btn-lg btn-primary btn-pill btn-cta"
        v-on:click="stamp()"
        v-html="this.lb_12"
      ></button>
    </div>
...
export default {
  ...
  methods: {
    ...
    checkVerify() {
      var self = this;
      self.verifyCounter++;
      if (self.verifyCounter == self.uploadedFiles.length)
        self.$emit("verify-completed", self.uploadedFiles);
    },
    verifybyURL(h) {
      var self = this;
