  ...
  <div id="app">
    <div v-if="limite != 0" class="alert alert-danger" role="alert">
      <p>
        ...
        <span v-html="this.lb_19"></span> <span v-html="this.limite"></span
        ><span v-html="this.lb_20"></span>
      </p>
    </div>
    <div>
      <div v-if="state == 'stamped'">
        <div v-for="(value, index) in allFiles" :key="index">
          <div
            v-if="value.status == 'already_stamped_by_this_TSA'"
            class="success-verify alert alert-success"
            role="alert"
          >
            <p>
              <span class="glyphicon glyphicon-ok" aria-hidden="true"></span>
              <span v-html="lb_00"></span> <b>{{ value.fileName }}</b
              ><span v-html="lb_21"></span> <span v-html="lb_04"></span>
              <b>{{ value.block }}</b>
            </p>
            <div class="copiar">
              <label
              ...
              <div class="input-group">
                <input
                  ...
                  type="textfield"
                  readonly
                  :value="getHashURL(index)"
                  :id="'id_' + index"
                />
                <span class="input-group-btn">
                  <button
                    class="btn btn-default btn-sm"
                    v-on:click="copiarURL(index)"
                  >
                    <span
                      class="glyphicon glyphicon-copy text-success"
                      aria-hidden="true"
                    ></span>
                    <span v-html="lb_15"></span>
                  </button>
                </span>
              </div>
            </div>
      ...
      <div
        v-if="state == 'failed-stamp'"
        class="fail-stamp alert alert-danger"
        role="alert"
      >
        <p>
          <span class="glyphicon glyphicon-remove" aria-hidden="true"></span>
          <span v-html="this.lb_02"></span> <b>{{ archivo }}</b>
        </p>
      </div>
        ...
        <div v-for="(value, index) in allFiles" :key="index">
          ...
          >
            <p>
              <span class="glyphicon glyphicon-ok" aria-hidden="true"></span>
              <span v-html="lb_00"></span> <b>{{ value.fileName }}</b>
              <span v-html="lb_03"></span>
            </p>
            <ul>
              <li v-for="(stamp, index) in value.stamps" :key="index">
                <span class="hash"
                  ><b>{{ stamp.whostamped }}</b></span
                >
                <span v-html="lb_04"></span> <b>{{ stamp.blocknumber }}</b>
                {{ convertTime(stamp.blocktimestamp) }}
              </li>
            </ul>
            <div class="copiar">
              <label
          ...
          <div v-else class="fail-verify alert alert-danger" role="alert">
            <p>
              <span
                ...
                aria-hidden="true"
              ></span>
              <span v-html="lb_05"></span> <b>{{ value.fileName }}</b>
            </p>
          </div>
        </div>
      </div>
      ...
      >
        <p>
          <span class="glyphicon glyphicon-ok" aria-hidden="true"></span>
          <span v-html="this.lb_00"></span> <b>{{ archivo }}</b>
          <span v-html="this.lb_03"></span>
        </p>
        <ul>
        ...
        <div class="copiar">
          ...
          <div class="input-group">
            <input
              ...
              type="textfield"
              readonly
              :value="getHashURL(null)"
              id="id_single"
            />
            <span class="input-group-btn">
              <button
                class="btn btn-default btn-sm"
                v-on:click="copiarURL(null)"
              >
                <span
                  class="glyphicon glyphicon-copy text-success"
                  aria-hidden="true"
      ...
      <div
        v-if="state == 'failed-verification'"
        class="fail-verify alert alert-danger"
      >
        <p>
          <span class="glyphicon glyphicon-remove" aria-hidden="true"></span>
          <span v-html="this.lb_05"></span> <b>{{ archivo }}</b>
        </p>
      </div>
...
<script>
import DropFile from "./DropFile2.vue";
